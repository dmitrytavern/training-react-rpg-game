...
const BlockCharacteristic = () => {
  ...
  const controller: PlayerCharacteristicController = store.getController(
    PlayerCharacteristicController
  )
  ...
  const disabledCharacteristicButton = (name: any) => {
    return controller.checkLimit(name) || availableCharacteristicPoints === 0
  }
