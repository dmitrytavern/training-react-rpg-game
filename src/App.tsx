...
const App = () => {
  ...
  const disabledCharacteristicButton = (name: any) => {
    return (
      characteristic.characteristicLimitReached(name) || characteristic.getAvailablePoints() === 0
    )
  }
  ...
  return (
    <div>
        ...
        {inventory.map((item, i) => (
          <li key={i}>
            ...
            <button
              ...
              disabled={player.favorites.exists(item.id)}
            >
              Add to favorite
            </button>
            <button
              onClick={() => unsetItemFromFavorite(item.id)}
              disabled={!player.favorites.exists(item.id)}
            >
              Unset from favorite
            </button>
      ...
      <button onClick={removeItem} disabled={!player.inventory.getItem(1)}>
        Remove 1 Sword
      </button>
